...
def generate_stress_tests_path(sqlite_path: str, query: str) -> List[str]:
    ...
    all_stress_tests = spans_dropped | tok_replaced
    all_stress_tests = [q for q in all_stress_tests if can_execute_path(empty_path, q)]
    results = []
    os.unlink(empty_path)
    for stress_test_query in all_stress_tests:
        if stress_test_query == query or equivalent_count(stress_test_query, query):
            continue
        results.append(stress_test_query)
    return results
