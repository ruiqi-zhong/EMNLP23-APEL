...
from sql_util.fuzz.date import DateFuzzer
from sql_util.fuzz.number import NumberFuzzer, isint
from sql_util.fuzz.bool import BoolFuzzer, BitFuzzer
from sql_util.fuzz.base import BaseFuzzer
from sql_util.fuzz.string import StringFuzzer
from sql_util.fuzz.special import special_cases, corrected_keys
from sql_util.fuzz.complete_foreign_keys import path2dep
...
from sql_util.fuzz.predefined_vals import col2values
import os
...
class DBFuzzer:
    ...
    def __init__(self, sqlite_path: str, values: Set[str], p: float = 0.2):
        self.sqlite_path = sqlite_path
        self.values = set(values)
        self.table_column_properties, self.child2parent, self.table_column2elements = get_all_db_info_path(sqlite_path)
        self.predifined_values = col2values.get(self.sqlite_path)

        # special handling of missing foreign keys in the database
        # otherwise fuzzing is extremely inefficient
        if sqlite_path in path2dep:
            self.child2parent = path2dep[sqlite_path]
    ...
    def initialize_fuzzer(self):
        for k in self.table_column_order[0]:
            ...
            elements = random.choices(elements, k=min(5, len(elements))) + list(self.values)
            if self.predifined_values is not None:
                elements = [e for e in elements if (k not in self.predifined_values or e in self.predifined_values[k])]
...
def fuzz_db_wrapper(args: Tuple[str, str, Set[str]]):
    orig_path, target_path, values = args
    print('now fuzzing based on database %s, target path %s.' % (orig_path, target_path))
    dbfuzzer = DBFuzzer(orig_path, values)
